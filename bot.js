...
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        ...
        msg.reply('Aleyküm Selam Hoşgeldin <a:752980827285225625:781797154032844811>');      
      } 
      }
    });
client.on('message', async (msg, member, guild) => {
...
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        if (msg.content.toLowerCase() === 'regual') {
        msg.reply('Regual Bne');      
      } 
      }
    });
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        ...
        msg.reply('Yapımcım Destek(`https://discord.gg/SafkZAg`)');      
      } 
      }
    });
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        if (msg.content.toLowerCase() === 'muffin ab') {
        msg.reply('Gacımız :d sjsjsjs');      
      } 
      }
    });
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        if (msg.content.toLowerCase() === 'putperest') {
        msg.reply('Caz Yapma Sahibim En Büyük');      
      } 
      }
    });
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        if (msg.content.toLowerCase() === 'film') {
        msg.reply('**1-https://jetfilmizle.live | 2-https://www.dizibox.vip**');      
      } 
      }
    });
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        ...
        msg.reply('**İşte Sitem:https://regual-com.glitch.me**');      
      } 
      }
    });
